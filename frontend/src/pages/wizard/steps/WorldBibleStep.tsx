...
interface Props {
  data: WizardData
  onNext: (patch: Pick<WizardData, 'worldBible'>) => void
  onBack: () => void
}
...
export default function WorldBibleStep({ data, onNext, onBack }: Props) {
  const [attrNames, setAttrNames] = useState<Record<string, string>>(
    data.worldBible.attributeNames
  )
  const [currencyName, setCurrencyName] = useState(data.worldBible.currencyName)
  ...
  function handleNext() {
    // Fall back to defaults for any blank field
  ...
  return (
    <div className="flex flex-col gap-5">
      <p className="text-sm text-zinc-400">
        ...
        Rename attributes to fit your setting — the underlying rules stay the same.
      </p>

      {/* Attribute rename grid */}
      <div>
